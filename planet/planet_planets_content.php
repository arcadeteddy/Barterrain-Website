...
$mysql = mysql_query("SELECT planets_array FROM planets WHERE id=$id");
...
////////// PLANETS!!! //////////
if($planets_array!=="")
{$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM planets_posts WHERE user_page_id IN ($PLANETS) AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM planets_member_posts WHERE user_page_id IN ($PLANETS) AND user_post_id='$id' AND delete_item='1')
...
while ($row = mysql_fetch_array ($mysql1))
	...
	while ($row =mysql_fetch_array ($mysql4))
		...
		$user_check_pic_comment = "../user_files/user$user_page_id_comment/profile_thumb.jpg";
		...
		$comment_profile_link='<a class="profile_link" href="../profile/profile.php?id='.$user_page_id_comment.'">'.$user_firstname_comment.' '.$user_lastname_comment.'</a>';
		
		if (($creator_display=="1")AND(in_array($user_post_id_comment,$creatorArray)))
			{$user_check_pic_comment="../user_files/user$user_id/profile_thumb.jpg";
			$user_default_pic_comment="../user_files/user0/default_profile_pic_thumb.png";
			if (file_exists($user_check_pic_comment))
				{$commenter_pic_comment="<a href='../profile/profile.php?id=".$user_page_id_comment."'><img src='$user_check_pic_comment#$cacheBuster' width='45px' height='45px' class='thumb_background'/></a>";}
			else
				{$commenter_pic_comment="<a href='../profile/profile.php?id=".$user_page_id_comment."'><img src='$user_default_pic_comment' width='45px' height='45px' class='thumb_background' /></a>";}
			$comment_profile_link="<a class='profile_link' href='../profile/profile.php?id=".$user_page_id_comment."'>".$user_firstname_comment." ".$user_lastname_comment."</a>";}
			...
			if (file_exists($user_check_pic_comment))
				{$commenter_pic_comment="<a href='../planet/planet.php?id=".$planet_id."'><img src='$user_check_pic_comment#$cacheBuster' width='45px' height='45px' class='thumb_background'/></a>";}
			else
				{$commenter_pic_comment="<a href='../planet/planet.php?id=".$planet_id."'><img src='$user_default_pic_comment' width='45px' height='45px' class='thumb_background' /></a>";}
...
while ($row = mysql_fetch_array ($mysql1))
	...
	else {$album_cover="<a href='../planet/planet.php?id=".$user_page_id."&force_album=".$album_id."'><img class='cover thumb_background' src='$album_default_pic' height='76px' width='130px'/></a>";}
...
while ($row = mysql_fetch_array ($mysql1))
	...
	else {$game_cover="<a href='../planet/planet.php?id=".$user_page_id."&force_game=".$game_id."'><img class='cover thumb_background' src='$game_default_pic' height='76px' width='130px'/></a>";}
...
while ($row = mysql_fetch_array ($mysql1))
	...
	while ($row =mysql_fetch_array ($mysql4))
		...
		if (($creator_display=="1")AND(in_array($user_post_id_comment,$creatorArray)))
			...
			$user_default_pic_comment="../planet_files/planet0/planet_thumb.png";
			if (file_exists($user_check_pic_comment))
				{$commenter_pic_comment="<a href='../planet/planet.php?id=".$user_page_id."'><img src='$user_check_pic_comment#$cacheBuster' width='45px' height='45px' class='thumb_background'/></a>";}
			else
				{$commenter_pic_comment="<a href='../planet/planet.php?id=".$user_page_id."'><img src='$user_default_pic_comment' width='45px' height='45px' class='thumb_background' /></a>";}
...
$(window).data('ajaxready5', true).scroll(function()
	...
	if($(window).scrollTop()>=$("#bottom_subscriptions").height()-$(document).height())
		...
		$.ajax({cache: false,url:"subscriptions_content.php?number="+number+"&cacheBuster="+cacheBuster+"&id="+id+"&ids="+ids,
