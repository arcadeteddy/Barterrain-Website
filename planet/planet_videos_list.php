...
else {$view_type="AND video_type!='b' AND video_type!='c'";}
...
unset($videos_array);
$mysql = mysql_query("SELECT id FROM planets_videos WHERE user_page_id=$id ".$view_type." AND delete_item='1' ORDER BY upload_date DESC");
...
if ($numRows>0)
	...
	if(($ids==$user_id)OR($ids==$user_post_id)OR(in_array($ids,$creatorArray))OR((in_array($ids,$adminArray))AND(!in_array($user_post_id,$creatorArray))AND($user_id!==$user_post_id)))
		...
		if(file_exists($video_pic))
			{$video_cover="<a href='#' onclick='return false' onmousedown='javascript:video_opener(".$ids.",".$id.",".$video_id.");'><img src='$video_pic#$cacheBuster' height='125px' width='213px' class='thumb_background'/></a>";}
		...
		$videosDisplayList .= '<div class="upload_box" id="upload_video_box'.$video_id.'">
								...
								<div class="upload_box3">
									<div class="like_love" id="like_love_videos'.$video_id.'">'.$hide_llc_front.''.$like_love_video.'
									<span class="dot_divider"> &middot; </span>'.$hide_llc_back.'<span class="post_date">'.$upload_date.'</span>'.$like_love2_video.'</div>
								</div>
							</div>';
		}
