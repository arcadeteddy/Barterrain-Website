...
////////// WALL!!! //////////
$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM planets_videos_posts WHERE video_id='$media_id' AND delete_item='1') ORDER BY datetime DESC LIMIT ".$number.",20");
...
$(window).data('ajaxready33', true).scroll(function()
	{if ($(window).data('ajaxready33') == false) return;
	if($(window).scrollTop()>=$("#bottom_media_video").height()-$(document).height())
		{$("div#expand_bottom_box").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready33', false);
		$.ajax({cache: false,url:"planet_videos_posts_content.php?number="+number+"&cacheBuster="+cacheBuster+"&id="+id+"&ids="+ids+"&video_id="+video_id,
			success:function(html)
				{if(html){$("#bottom_media_video").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_box").show();}
				 $(window).data('ajaxready33', true);}
			});
		}
	});
