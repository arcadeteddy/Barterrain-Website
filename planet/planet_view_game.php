...
$mysql_views = mysql_query("SELECT unique_views_planets_games FROM members_planets WHERE id=$ids LIMIT 1");
while($row = mysql_fetch_array($mysql_views))
	{$unique_views_planets_games = $row['unique_views_planets_games'];
	$unique_views_planets_games_explode = explode(",",$unique_views_planets_games);
	$unique_views_planets_games_slice = array_slice($unique_views_planets_games_explode, 0, 1);}
$mysql_views = mysql_query("SELECT unique_views, total_views FROM planets_games WHERE id=$game_id LIMIT 1");
while($row = mysql_fetch_array($mysql_views))
	{$unique_views = $row['unique_views'];
	$total_views = $row['total_views'];}
if (!in_array($game_id,$unique_views_planets_games_explode))
	{$unique_views=$unique_views+1;
	$total_views=$total_views+1;
	if ($unique_views_planets_games != ""){$unique_views_planets_games = "$game_id,$unique_views_planets_games";}
	else {$unique_views_planets_games = "$game_id";}}
else if (!in_array($game_id,$unique_views_planets_games_slice))
	{$total_views=$total_views+1;
	foreach ($unique_views_planets_games_explode as $key => $value) 
		{if ($value == $game_id)
			{unset($unique_views_planets_games_explode[$key]);}}
	$unique_views_planets_games = implode(",",$unique_views_planets_games_explode);
	if ($unique_views_planets_games != ""){$unique_views_planets_games = "$game_id,$unique_views_planets_games";}
	else {$unique_views_planets_games = "$game_id";}}
mysql_query("UPDATE planets_games SET unique_views='$unique_views', total_views='$total_views' WHERE id='$game_id'");
mysql_query("UPDATE members_planets SET unique_views_planets_games='$unique_views_planets_games' WHERE id='$ids'");

$mysql2 = mysql_query("SELECT * FROM planets_games WHERE id=$game_id");
...
while ($row = mysql_fetch_array ($mysql2))
	...
	if(($ids==$user_id)OR($ids==$user_post_id)OR(in_array($ids,$creatorArray))OR((in_array($ids,$adminArray))AND(!in_array($user_post_id,$creatorArray))AND($user_id!==$user_post_id)))
		{$delete_button_game="<div class='option_box' id='delete_games".$game_id."'><a title='Delete' href='#' class='top_options_delete1' onclick='return false' onmousedown='javascript:delete_media(".$game_id.",".$comment_type.");'></a></div>";}
		else{$delete_button_game="";}
	if($game_description==""){$game_description="<br/>";}
	else{$game_description="<br/><br/><b>Description: </b>".$game_description."<br/>";}
	if ($game_name=="")
		{$game_name == "Untitled";}

$check_size="../planet_files/planet$id/planet_game_".$game_id."/planet_game_".$game_id.".swf";
...
function media_post_form()
	...
	if (post_field.val() == '')
		...
		$("#interactive_error").html('').show();
		$.post(url,{id:id,ids:ids,media_post:post_field.val(),type:post_type.val(),media_id:game_id,media_type:media_type,typex:type,thisWipit:thisRandNum},function(data)
...
<object style="width:755px;height:<?php echo $height; ?>px;margin:5px 0px;z-index:-100000;" scale="exactfit" wmode="transparent">
	<param name="movie" value="../planet_files/planet<?php echo $id; ?>/planet_game_<?php echo $game_id; ?>/planet_game_<?php echo $game_id; ?>.swf"></param> 
	<param name="wmode" value="opaque"></param> 
</object>
<?php echo $game_description;?><br/><b>Unique Views: </b><?php echo $unique_views;?> | <b>Total Views: </b><?php echo $total_views;?><br/><br/>
...
<div class="bottom_wall" id="bottom_media_game">
	<?php include "planet_games_posts_content.php";?>
</div>
