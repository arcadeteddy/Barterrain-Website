...
$mysql = mysql_query("SELECT family_array, friend_array FROM members WHERE id=$ids");
...
////////// WALL!!! //////////
$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM posts WHERE user_post_id IN ($FF_News) AND delete_item='1')
					UNION ALL(SELECT id, note_date AS datetime, union_type FROM notes WHERE user_id IN ($FF_News) AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM albums WHERE user_id IN ($FF_News) AND delete_item='1')
					...
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM albums_posts WHERE user_id IN ($FF_News) AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM games_posts WHERE user_id IN ($FF_News) AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM videos_posts WHERE user_id IN ($FF_News) AND delete_item='1')
					UNION ALL(SELECT id, create_date AS datetime, union_type FROM planets WHERE user_id IN ($FF_News) AND delete_item='1') 
...
else if (($numRows=="0")AND($number=="0"))
	{$DisplayList = "<div>
			<div class='post_box'>
			<div class='post_box1'><a class='profile_link' href='http://www.barterrain.com/planets/planets.php?id=1'><img src='../planet_files/planet1/planet_thumb.jpg' width='55px' height='55px'/></a></div>
			<div class='post_box3'>
			<div class='post_box2'><a class='profile_link' href='../planets/planets.php?id=1'>Barterrain</a></div>
			<div class='post_box2'>
				Welcome to Barterrain! You can <a href='http://www.barterrain.com/friends/friends.php' onClick='spiral2()' class='body'>Find Friends</a> or 
				<a href='http://www.barterrain.com/friends/friends.php?friends=invite' onClick='spiral2()' class='body'>Invite Friends</a> to fill up this empty page. You can visit some 
				<a href='http://www.barterrain.com/planets/planets.php' onClick='spiral()' class='body'>Planets</a> to see what others have been up to! Enjoy your stay at Barterrain!
			</div>
			</div></div></div>";
	}
...
$(window).data('ajaxready1', true).scroll(function()
	...
	if($(window).scrollTop()>=$("#bottom_news").height()-$(document).height())
		{$("div#expand_bottom_box").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready1', false);
		$.ajax({cache: false,url:"news_content.php?number="+number+"&cacheBuster="+cacheBuster+"&id="+id+"&ids="+ids,
			success:function(html)
				{if(html){$("#bottom_news").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_box").show();}
...
window.onload=function()
		{$('#inside_page_middle').css('min-height', $('#inside_page_left').height()+150+'px');};
</script>