...
$mysql = mysql_query("SELECT family_array FROM members WHERE id=$ids");
while($row = mysql_fetch_array($mysql))
	{$family_array = $row['family_array'];
	$familyArray = explode(",",$family_array);
	$Family = join(',',$familyArray);}
	
if(!isset($family_id)){$family_id=$ids;}
	
////////// SUBSCRIPTIONS!!! //////////
if($family_array!=="")
{if ((isset($family_id))AND($family_id!=$ids)){$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM posts WHERE (user_page_id='$family_id' OR user_post_id='$family_id') AND union_type='posts' AND post_type!='$view_type') 
					UNION ALL(SELECT id, note_date AS datetime, union_type FROM notes WHERE user_id='$family_id' AND delete_item='1' AND note_type!='$view_type')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM albums WHERE user_id='$family_id' AND delete_item='1' AND album_type!='$view_type')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM images_walls WHERE user_id='$family_id' AND delete_item='1' AND images_wall_type!='$view_type')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM videos WHERE user_id='$family_id' AND delete_item='1' AND video_type!='$view_type')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM games WHERE user_id='$family_id' AND delete_item='1' AND game_type!='$view_type')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM albums_posts WHERE user_id='$family_id' AND delete_item='1' AND albums_post_type!='$view_type')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM games_posts WHERE user_id ='$family_id' AND delete_item='1' AND games_post_type!='$view_type')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM videos_posts WHERE user_id ='$family_id' AND delete_item='1' AND videos_post_type!='$view_type')
					UNION ALL(SELECT id, create_date AS datetime, union_type FROM planets WHERE user_id='$family_id' AND delete_item='1' AND planet_type!='$view_type')
					ORDER BY datetime DESC LIMIT ".$number.",20");}
else {$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM posts WHERE (user_post_id IN ($Family) OR user_page_id IN ($Family)) AND delete_item='1' AND post_type!='$view_type')
					UNION ALL(SELECT id, note_date AS datetime, union_type FROM notes WHERE user_id IN ($Family) AND delete_item='1' AND note_type!='$view_type')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM albums WHERE user_id IN ($Family) AND delete_item='1' AND album_type!='$view_type')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM images_walls WHERE user_id IN ($Family) AND delete_item='1' AND images_wall_type!='$view_type')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM videos WHERE user_id IN ($Family) AND delete_item='1' AND video_type!='$view_type')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM games WHERE user_id IN ($Family) AND delete_item='1' AND game_type!='$view_type')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM albums_posts WHERE user_id IN ($Family) AND delete_item='1' AND albums_post_type!='$view_type')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM games_posts WHERE user_id IN ($Family) AND delete_item='1' AND games_post_type!='$view_type')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM videos_posts WHERE user_id IN ($Family) AND delete_item='1' AND videos_post_type!='$view_type')
					UNION ALL(SELECT id, create_date AS datetime, union_type FROM planets WHERE user_id IN ($Family) AND delete_item='1' AND planet_type!='$view_type') 
					ORDER BY datetime DESC LIMIT ".$number.",20");}
...
while ($row = mysql_fetch_array ($mysql1))
	...
	if (file_exists($user_check_pic)) 
		{$poster_pic = "<img  src='$user_check_pic#$cacheBuster' width='55px' class='thumb_background'/>";} 
	else {$poster_pic = "<img src='$user_default_pic' width='55px' class='thumb_background'/>";}
	$comment_space="";
	...
	if(($ids==$user_page_id)OR($ids==$user_post_id))
		{$delete_button_post="<div class='option_box' id='delete_posts".$post_id."'><a href='#' title='Delete' class='delete0' onclick='return false' onmousedown='javascript:delete_post1(".$post_id.",".$comment_type.");'></a></div>";}
...
$(window).data('ajaxready9', true).scroll(function()
	{if ($(window).data('ajaxready9') == false) return;
	if($(window).scrollTop()>=$("#bottom_family").height()-$(document).height())
		{$("div#expand_bottom_box").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready9', false);
		$.ajax({cache: false,url:"family_content.php?number="+number+"&cacheBuster="+cacheBuster+"&id="+id+"&ids="+ids+"&family_id="+family_id,
			success:function(html)
				{if(html){$("#bottom_family").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_box").show();}
				 $(window).data('ajaxready9', true);}
			});
		}
	});
