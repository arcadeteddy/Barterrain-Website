...
if(isset($_GET['number']))
	...
	$ids=$_GET['ids'];
	$id=$_GET['id'];
	$subscriptions_id=$_GET['subscriptions_id'];}
	
if ((!isset($id))OR(!isset($ids)))
	{header("Location: ../index.php");exit();}
ob_flush();
	
$mysql = mysql_query("SELECT subscriptions_array FROM members WHERE id=$ids");
...
if(!isset($subscriptions_id)){$subscriptions_id=$ids;}
	
////////// SUBSCRIPTIONS!!! //////////
if(($subscriptions_array!=="")OR(isset($subscriptions_id)))
{if ((isset($subscriptions_id))AND($subscriptions_id!=$ids)){$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM posts WHERE (user_page_id='$subscriptions_id' OR user_post_id='$subscriptions_id') AND delete_item='1') 
					UNION ALL(SELECT id, note_date AS datetime, union_type FROM notes WHERE user_id='$subscriptions_id' AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM albums WHERE user_id='$subscriptions_id' AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM images_walls WHERE user_id='$subscriptions_id' AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM videos WHERE user_id='$subscriptions_id' AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM games WHERE user_id='$subscriptions_id' AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM albums_posts WHERE user_id='$subscriptions_id' AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM games_posts WHERE user_id ='$subscriptions_id' AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM videos_posts WHERE user_id ='$subscriptions_id' AND delete_item='1')
					UNION ALL(SELECT id, create_date AS datetime, union_type FROM planets WHERE user_id ='$subscriptions_id' AND delete_item='1') 
					ORDER BY datetime DESC LIMIT ".$number.",20");}
else {$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM posts WHERE (user_post_id IN ($Subscriptions) OR user_page_id IN ($Subscriptions)) AND delete_item='1')
					UNION ALL(SELECT id, note_date AS datetime, union_type FROM notes WHERE user_id IN ($Subscriptions) AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM albums WHERE user_id IN ($Subscriptions) AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM images_walls WHERE user_id IN ($Subscriptions) AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM videos WHERE user_id IN ($Subscriptions) AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM games WHERE user_id IN ($Subscriptions) AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM albums_posts WHERE user_id IN ($Subscriptions) AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM games_posts WHERE user_id IN ($Subscriptions) AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM videos_posts WHERE user_id IN ($Subscriptions) AND delete_item='1')
					UNION ALL(SELECT id, create_date AS datetime, union_type FROM planets WHERE user_id IN ($Subscriptions) AND delete_item='1') 
					ORDER BY datetime DESC LIMIT ".$number.",20");}
...
var subscriptions_id = "<?php echo $subscriptions_id; ?>";
$(window).data('ajaxready10', true).scroll(function()
	{if ($(window).data('ajaxready10') == false) return;
	if($(window).scrollTop()>=$("#bottom_subscriptions").height()-$(document).height())
		{$("div#expand_bottom_box").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready10', false);
		$.ajax({cache: false,url:"subscriptions_content.php?number="+number+"&cacheBuster="+cacheBuster+"&id="+id+"&ids="+ids+"&subscriptions_id="+subscriptions_id,
			success:function(html)
				{if(html){$("#bottom_subscriptions").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_box").show();}
				 $(window).data('ajaxready10', true);}
			});
		}
	});
