...
if((isset($_POST['item_id']))AND(isset($_POST['comment_type'])))
	{$item_id=$_POST['item_id'];
	$comment_type=$_POST['comment_type'];
	if($comment_type!=="link_creates"){$mysql_create = mysql_query("SELECT user_page_id FROM ".$typex."_".$comment_type." WHERE id='$item_id'");}
	else{$mysql_create = mysql_query("SELECT user_page_id FROM link_creates WHERE id='$item_id'");}
	while($row = mysql_fetch_array($mysql_create))
		{$id = $row['user_page_id'];}}

	$mysql = mysql_query("SELECT ".$typex_s."_name AS create_name, creator_display, user_id, creator_array, admin_array, member_array FROM ".$typex." WHERE id=$id");
...
if ($_POST["interactive"] == "comment_point1")
	...
	$comment_type=$_POST['comment_type'];
	if($comment_type=="link_creates"){$mysql = mysql_query("SELECT point_array FROM link_creates_comments WHERE id='$comment_id' LIMIT 1");}
	else{$mysql = mysql_query("SELECT point_array FROM ".$typex."_".$comment_type."_comments WHERE id='$comment_id' LIMIT 1");}
	while ($row = mysql_fetch_array ($mysql))
...
if ($_POST["interactive"] == "comment_point2")
	...
	if($comment_type=="link_creates"){$mysql = mysql_query("SELECT ".$comment_typex_s."_id AS item_id, user_post_id,point_array FROM link_creates_comments WHERE id='$comment_id' LIMIT 1");}
	else{$mysql = mysql_query("SELECT ".$comment_typex_s."_id AS item_id, user_post_id, point_array FROM ".$typex."_".$comment_type."_comments WHERE id='$comment_id' LIMIT 1");}
	while ($row = mysql_fetch_array ($mysql))
		{$item_id = $row['item_id'];
	...
	if($user_lose_points>9)
		...
		mysql_query("UPDATE point_totals_comments SET ".$typex."_".$comment_type_total."_plus = ".$typex."_".$comment_type_total."_plus + 10 WHERE id='$user_id'");
		mysql_query("UPDATE point_totals_comments SET ".$typex."_".$comment_type_total."_minus = ".$typex."_".$comment_type_total."_minus + 10 WHERE id = '$ids'");
		if($comment_type=="link_creates"){mysql_query("UPDATE link_creates_comments SET point_array='$point_array' WHERE id='$comment_id'");}
		else{mysql_query("UPDATE ".$typex."_".$comment_type."_comments SET point_array='$point_array' WHERE id='$comment_id'");}
		
		if($comment_type=="link_creates"){$mysql_create_id = mysql_query("SELECT user_page_id FROM link_creates WHERE id='$item_id' LIMIT 1");}
		else{$mysql_create_id = mysql_query("SELECT user_page_id FROM ".$typex."_".$comment_type." WHERE id='$item_id' LIMIT 1");}
		...
		{$point_array_count = $point_array_count*10;
		echo "<a href='#' title='Not Enough Points!' class='point' onclick='return false' onmousedown='javascript:comment_point1(".$ids.",".$comment_id.",".$comment_type.");'></a><b title='Not Enough Points!' class='point2'>".$point_array_count."</b>";}
exit();}

///////////////// COMMENT CHANGER /////////////////
if ($_POST["interactive"] == "Comment")
	{$item_id=$_POST['item_id'];
	$comment_type2=$_POST['comment_type'];
	$typex=$_POST['typex'];
	$comment_type= json_encode($comment_type2);
...
if ($_POST["interactive"] == "delete_comment2")
	...
	$item_id=$_POST['item_id'];
	$typex=$_POST['typex'];
	if($comment_type=="link_creates"){$mysql=mysql_query("SELECT comment_array FROM link_creates WHERE id='$item_id' LIMIT 1");}
	else{$mysql=mysql_query("SELECT comment_array FROM ".$typex."_".$comment_type." WHERE id='$item_id' LIMIT 1");}
	while ($row =mysql_fetch_array ($mysql))
	...
	if($comment_type=="link_creates")
		{mysql_query("UPDATE link_creates_comments SET delete_comment='0' WHERE id='$comment_id'");}
	else
		{mysql_query("UPDATE ".$typex."_".$comment_type."_comments SET delete_comment='0' WHERE id='$comment_id'");}
	echo '';
	exit();}
...
if ($_POST["interactive"] == "expand_comments")
	...
	if($comment_type=="link_creates"){$mysql3 = mysql_query("SELECT * FROM link_creates_comments WHERE ".$comment_type_s."_id='$item_id' AND delete_comment='1' ORDER BY comment_date DESC LIMIT 30");}
	else{$mysql3 = mysql_query("SELECT * FROM ".$typex."_".$comment_type."_comments WHERE ".$comment_type_s."_id='$item_id' AND delete_comment='1' ORDER BY comment_date DESC LIMIT 30") or die (mysql_error());}
	$comment_type= json_encode($comment_type);
	...
	$mysql1 = mysql_query("SELECT planet_name, creator_array FROM planets WHERE id='$user_page_id_comment'");
	...
	if (($like_array_comment !="")AND($love_array_comment !=""))
		{$like_love2_comment = "<span class='dot_divider'> &middot; </span><a href='#' title='Like Array' class='post_link display_button' onclick='return false' onmousedown='javascript:display_like_array_comment(".$id.",".$ids.",".$comment_id.",".$comment_type.");'>".$like_array_count_comment."</a> <img src='blank.gif' class='like_count'/><span class='dot_divider'> &middot; </span>
						<a href='#' title='Love Array' class='post_link display_button' onclick='return false' onmousedown='javascript:display_love_array_comment(".$id.",".$ids.",".$comment_id.",".$comment_type.");'>".$love_array_count_comment."</a> <img src='blank.gif' class='love_count'/>";}
	else if ($like_array_comment !="")
		{$like_love2_comment = "<span class='dot_divider'> &middot; </span><a href='#' title='Like Array' class='post_link display_button' onclick='return false' onmousedown='javascript:display_like_array_comment(".$id.",".$ids.",".$comment_id.",".$comment_type.");'>".$like_array_count_comment."</a> <img src='blank.gif' class='like_count'/>";}
	else if ($love_array_comment !="")
		{$like_love2_comment = "<span class='dot_divider'> &middot; </span><a href='#' title='Love Array' class='post_link display_button' onclick='return false' onmousedown='javascript:display_love_array_comment(".$id.",".$ids.",".$comment_id.",".$comment_type.");'>".$love_array_count_comment."</a> <img src='blank.gif' class='love_count'/>";}
	else
		{$like_love2_comment = "";}
	...
	else{$comment_point="<div title='Point Array' class='option_box2'><a href='#' class='point display_button' onclick='return false'  onmousedown='javascript:display_point_array_comment(".$id.",".$ids.",".$comment_id.",".$comment_type.");'></a><a href='#' title='Point Array' class='body display_button' onclick='return false' onmousedown='javascript:display_point_array_comment(".$id.",".$ids.",".$comment_id.",".$comment_type.");'><b class='point3'>".$comment_point_array_count."</b></a></div>";}
						
	$mysql4=mysql_query("SELECT alias, alias_activation FROM members_".$typex." WHERE id='$user_post_id_comment' LIMIT 1");
	...
	while ($row =mysql_fetch_array ($mysql4))
		...
		if ($creator_display=="1")
			...
			{$user_check_pic_comment="../".$typex_s."_files/".$typex_s."$id/".$typex_s."_thumb.jpg";
			$user_default_pic_comment="../".$typex_s."_files/".$typex_s."0/".$typex_s."_thumb.png";
			if (file_exists($user_check_pic_comment))
				{$commenter_pic_comment="<img src='$user_check_pic_comment#$cacheBuster' width='45px' class='thumb_background'/>";}
			else
				{$commenter_pic_comment="<img src='$user_default_pic_comment' width='45px' class='thumb_background' />";}
			$comment_profile_link="<a class='profile_link' href='../".$typex_s."/".$typex_s.".php?id=".$id."'>".$planet_name."</a>";
			$commenter_pic_comment="<a href='../".$typex_s."/".$typex_s.".php?id=".$id."'>".$commenter_pic_comment."</a>";}
	...
	$DisplayList .='
	<div id="comment_list_'.$comment_type2.''.$comment_id.'">
	<div class="comment_list">
		<div class="comment_1"><a class="profile_link" href="http://www.barterrain.com/profile/profile.php?id='.$user_page_id_comment.'">'.$commenter_pic_comment.'</a></div>
		<div class="comment_2">
...
if(isset($_POST['the_comment']))
	...
	if($comment_type=="link_creates"){$mysql_comment_array = mysql_query("SELECT user_page_id,comment_array FROM link_creates WHERE id='$item_id' LIMIT 1");}
	else{$mysql_comment_array = mysql_query("SELECT user_page_id,comment_array FROM ".$typex."_".$comment_type." WHERE id='$item_id' LIMIT 1");}
	while ($row=mysql_fetch_array($mysql_comment_array))
		{$user_page_id = $row['user_page_id'];
		$comment_array = $row['comment_array'];}
	$mysql1 = mysql_query("SELECT planet_name, creator_array FROM planets WHERE id='$user_page_id'");
	while($row = mysql_fetch_array($mysql1))
		{$planet_name = $row['planet_name'];
		$creator_array = $row['creator_array'];
		$creatorArray = explode(",",$creator_array);}
// Insert into database.
	if($comment_type=="link_creates"){$mysql1 = mysql_query("INSERT INTO link_creates_comments (".$comment_type_s."_id,user_page_id, user_post_id, the_comment, comment_date) VALUES ('$item_id','$user_page_id', '$ids', '$comment', UTC_TIMESTAMP())") or die (mysql_error());}
	else{$mysql1 = mysql_query("INSERT INTO ".$typex."_".$comment_type."_comments (".$comment_type_s."_id,user_page_id, user_post_id, the_comment, comment_date) VALUES ('$item_id','$user_page_id', '$ids', '$comment', UTC_TIMESTAMP())") or die (mysql_error());}
	$comment_id=mysql_insert_id();
	if ($comment_array != "")
		{$comment_array_new = "$comment_array,$comment_id";}
	else {$comment_array_new = "$comment_id";}
	if($comment_type=="link_creates"){$mysql2 = mysql_query("UPDATE link_creates SET comment_array = '$comment_array_new' WHERE id='$item_id'") or die (mysql_error());}
	else{$mysql2 = mysql_query("UPDATE ".$typex."_".$comment_type." SET comment_array = '$comment_array_new' WHERE id='$item_id'") or die (mysql_error());}
// Refresh the comments.
	$comment_type2=$comment_type;
	if($comment_type=="link_creates"){$mysql3 = mysql_query("SELECT * FROM link_creates_comments WHERE ".$comment_type_s."_id='$item_id' AND delete_comment='1' ORDER BY comment_date DESC LIMIT 3");}
	else{$mysql3 = mysql_query("SELECT * FROM ".$typex."_".$comment_type."_comments WHERE ".$comment_type_s."_id='$item_id' AND delete_comment='1' ORDER BY comment_date DESC LIMIT 3");}
	$comment_type= json_encode($comment_type);
	while ($row = mysql_fetch_array ($mysql3))
		{$comment_id = $row['id'];
		$user_page_id_comment = $row['user_page_id'];
...
// Email Content
	$mysql = mysql_query("SELECT user_id, planet_name, creator_array FROM planets WHERE id='$user_page_id' LIMIT 1");
	while ($row = mysql_fetch_array ($mysql))
		{$user_id = $row['user_id'];
		$planet_name = $row['planet_name'];
		$creator_array = $row['creator_array'];
	...
	if(($ids!==$user_id)AND(!in_array($ids,$creator_array)))
		{$mysql1 = mysql_query("SELECT planet_email1, planet_email2, email_comment_notification_activation FROM members_planets WHERE id='$user_id'");
		while($row = mysql_fetch_array($mysql1))
	...
	if (($email_comment_notification_activation=="1")AND($planet_email!==""))
			{$subject = "New Planet Comment From ".$profile_text." [".date("F jS, Y | H:i:s")."]";
        	   				...
        	   				<tr style='position:relative;'>
								...
								<table><tr>
									...
									<td style='text-align:left;width:450px;float:left;vertical-align:top;padding-left:15px;'>
										".$profile_link."
										<br/><font style='font:16px helvetica, sans-serif;margin:0px;padding:0px;'> New Planet Comment: <i>\"".$comment."\"</i></font>
									</td>
								</tr></table>
            					</td>
								<td style='width:15px;background-color:#FFFFFF;'></td>
			...
			mail($planet_email,$subject,$message,$headers,'-fcomment@barterrain.com');
			}
		}
exit();}
///////////////// COMMENTING END /////////////////
