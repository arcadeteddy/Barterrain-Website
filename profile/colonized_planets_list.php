...
$mysql1 = mysql_query("SELECT planets_array FROM members WHERE id='$id'");
...
if ($planets_array != "")
	...
	if($planets_planets>1)
		{for($x=1;$x<=$planets_planets;$x++)
			...
			else if($x==$planets_planets){$list_planets.="<a href='#' onclick='return false' onmousedown='javascript:colonized_planets_list(".$x.",".$id.",".$ids.");' class='bold'>".$x."</a>";}
			else {$list_planets.="<a href='#' onclick='return false' onmousedown='javascript:colonized_planets_list(".$x.",".$id.",".$ids.");' class='bold'>".$x."</a><span class='dot_pagination'> &bull; </span>";}
			}
		}
	// End Pagination
	$planets_list .= "<div class='middle_bars'>
					<div class='float_left'><span class='heading_list'><img src='blank.gif' width='1px' height='1px' class='planets_colonized_lists'>Colonized Planets (".$planets_count.")</span></div>
...
// friends Pagination
function colonized_planets_list(a,b,c)
	{$.post("colonized_planets_list.php?page="+a,{x:a,id:b,ids:c,cacheBuster:cacheBuster},function(data) 
		{$("#colonized_planets_list").html(data).show()});	
	}
</script>