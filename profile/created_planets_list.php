...
if ($planets_array != "")
	...
	if($planets_planets>1)
		{for($x=1;$x<=$planets_planets;$x++)
			...
			else if($x==$planets_planets){$list_planets.="<a href='#' onclick='return false' onmousedown='javascript:created_planets_list(".$x.",".$id.",".$ids.");' class='bold'>".$x."</a>";}
			else {$list_planets.="<a href='#' onclick='return false' onmousedown='javascript:created_planets_list(".$x.",".$id.",".$ids.");' class='bold'>".$x."</a><span class='dot_pagination'> &bull; </span>";}
			}
		}
	// End Pagination
	$planets_list .= "<br/><div class='middle_bars'>
					<div class='float_left'><span class='heading_list'><img src='blank.gif' width='1px' height='1px' class='planets_lists'>Created Planets (".$planets_count.")</span></div>
	...
	if (($like_array_planet !="")AND($love_array_planet !=""))
		...
		$planets_list .="<div class='list_wrap2'>
						...
						<div class='list_wrap_3'><div class='profile_link'><a class='profile_link' href='../planet/planet.php?id=".$planet_id."'><b>".$planet_name."</b></a></div>
							...
							<span class='places'>".$planet_type."</span>
						</div>
						</div>";
			}
		}
...
// friends Pagination
function created_planets_list(a,b,c)
	{$.post("created_planets_list.php?page="+a,{x:a,id:b,ids:c,cacheBuster:cacheBuster},function(data) 
		{$("#created_planets_list").html(data).show()});	
	}
</script>