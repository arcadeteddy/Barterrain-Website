...
$mysql = mysql_query("SELECT planets_array FROM members WHERE id='$id' LIMIT 1");
	...
	if($images_count==1)
		{foreach ($imagesArray as $key => $value)
			...
			$row=mysql_fetch_assoc($mysql_gif);
				{$ext=$row['ext'];}
			if ($ext=="gif") {$black_gif="<div style='float:left;'><img class='black_gif' src='barterrain_profile_images/black.png' height='415px' width='415px'/></div><table class='black_gif' height='415px' width='415px'><tr><td><span class='black_gif'>GIF</span></td></tr></table>";}
			$i++;$images_pic="../planet_files/planet$user_page_id/planet_album_thumbs_$album_id/planet_album_".$album_id."_thumb_".$value.".jpg";
			$pictures="<div style='float:left;'><a href='#' onclick='return false' class='display_image_button' onmousedown='javascript:display_image(".$planet_id.",".$ids.",".$value.");'>".$black_gif."<img class='images_walls' src='$images_pic' height='415px' width='415px'/></a></div>";}}
		...
		{foreach ($imagesArray as $key => $value)
			...
			$row=mysql_fetch_assoc($mysql_gif);
				{$ext=$row['ext'];}
			if ($ext=="gif") {$black_gif="<div style='float:left;'><img class='black_gif' src='barterrain_profile_images/black.png' height='203px' width='203px'/></div><table class='black_gif' height='203px' width='203px'><tr><td><span class='black_gif'>GIF</span></td></tr></table>";}
			$i++;$images_pic="../planet_files/planet$user_page_id/planet_album_thumbs_$album_id/planet_album_".$album_id."_thumb_".$value.".jpg";
			$pictures .="<div style='float:left;'><a href='#' onclick='return false' class='display_image_button' onmousedown='javascript:display_image(".$planet_id.",".$ids.",".$value.");'>".$black_gif."<img class='images_walls' src='$images_pic' height='203px' width='203px'/></a></div>";}}
		...
		{foreach ($imagesArray as $key => $value)
			...
			$i++;$images_pic="../planet_files/planet$user_page_id/planet_album_thumbs_$album_id/planet_album_".$album_id."_thumb_".$value.".jpg";
			$pictures .="<div style='float:left;'><a href='#' onclick='return false' class='display_image_button' onmousedown='javascript:display_image(".$planet_id.",".$ids.",".$value.");'>".$black_gif."<img class='images_walls' src='$images_pic' height='133px' width='133px'/></a></div>";}}
...
while ($row = mysql_fetch_array ($mysql1))
	...
	if (file_exists($game_pic))
		{$game_cover="<a href='../planet/planet.php?id=".$user_page_id."&force_game=".$game_id."'><img class='cover thumb_background' src='$game_pic' height='76px' width='130px'/></a>";}
...
$(window).data('ajaxready4', true).scroll(function()
	{if ($(window).data('ajaxready4') == false) return;
	if($(window).scrollTop()>=$("#bottom_planets").height()-$(document).height())
		{$("div#expand_bottom_box").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready4', false);
		$.ajax({cache: false,url:"planets_content.php?number="+number+"&cacheBuster="+cacheBuster+"&id="+id+"&ids="+ids,
			success:function(html)
				{if(html){$("#bottom_planets").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_box").show();}
				 $(window).data('ajaxready4', true);}
			});
		}
	});
