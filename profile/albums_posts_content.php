...
////////// WALL!!! //////////
$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM albums_posts WHERE album_id='$media_id' AND delete_item='1') ORDER BY datetime DESC LIMIT ".$number.",20");
...
$(window).data('ajaxready21', true).scroll(function()
	...
	if($(window).scrollTop()>=$("#bottom_media_album").height()-$(document).height())
		...
		$.ajax({cache: false,url:"albums_posts_content.php?number="+number+"&cacheBuster="+cacheBuster+"&id="+id+"&ids="+ids+"&album_id="+album_id,
