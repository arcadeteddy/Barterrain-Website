...
$mysql_views = mysql_query("SELECT unique_views_profile_albums FROM members WHERE id=$ids LIMIT 1");
while($row = mysql_fetch_array($mysql_views))
	{$unique_views_profile_albums = $row['unique_views_profile_albums'];
	$unique_views_profile_albums_explode = explode(",",$unique_views_profile_albums);
	$unique_views_profile_albums_slice = array_slice($unique_views_profile_albums_explode, 0, 1);}
$mysql_views = mysql_query("SELECT unique_views, total_views FROM albums WHERE id=$album_id LIMIT 1");
while($row = mysql_fetch_array($mysql_views))
	{$unique_views = $row['unique_views'];
	$total_views = $row['total_views'];}
if (!in_array($album_id,$unique_views_profile_albums_explode))
	{$unique_views=$unique_views+1;
	$total_views=$total_views+1;
	if ($unique_views_profile_albums != ""){$unique_views_profile_albums = "$album_id,$unique_views_profile_albums";}
	else {$unique_views_profile_albums = "$album_id";}}
else if (!in_array($album_id,$unique_views_profile_albums_slice))
	{$total_views=$total_views+1;
	foreach ($unique_views_profile_albums_explode as $key => $value) 
		{if ($value == $album_id)
			{unset($unique_views_profile_albums_explode[$key]);}}
	$unique_views_profile_albums = implode(",",$unique_views_profile_albums_explode);
	if ($unique_views_profile_albums != ""){$unique_views_profile_albums = "$album_id,$unique_views_profile_albums";}
	else {$unique_views_profile_albums = "$album_id";}}
mysql_query("UPDATE albums SET unique_views='$unique_views', total_views='$total_views' WHERE id='$album_id'");
mysql_query("UPDATE members SET unique_views_profile_albums='$unique_views_profile_albums' WHERE id='$ids'");

$mysql2 = mysql_query("SELECT id, user_id, album_name FROM albums WHERE id=$album_id");
while($row = mysql_fetch_array($mysql2))
	{$user_id = $row['user_id'];
	$album_name = $row['album_name'];}
$mysql2 = mysql_query("SELECT * FROM albums WHERE id=$album_id");
...
while ($row = mysql_fetch_array ($mysql2))
	...
	else{$point="<div title='Point Array' class='option_box2'><a href='#' title='Point Array' class='point0 display_button' onclick='return false' onmousedown='javascript:display_point_array(".$id.",".$ids.",".$item_id.",".$comment_type.");'></a><a href='#' title='Point Array' class='body display_button' onclick='return false' onmousedown='javascript:display_point_array(".$id.",".$ids.",".$item_id.",".$comment_type.");'><b class='point3'>".$point_array_count_album."</b></a></div>";}
	if($ids==$user_id)
		{$delete_button_album="<div class='option_box' id='delete_albums".$album_id."'><a href='#' title='Delete' class='delete0' onclick='return false' onmousedown='javascript:delete_media(".$album_id.",".$comment_type.");'></a></div>";}
...
function media_post_form()
	...
	if (post_field.val() == '')
		...
		$("#interactive_error").html('').show();
		$.post(url,{id:id,ids:ids,media_post:post_field.val(),type:post_type.val(),media_id:album_id,media_type:media_type,thisWipit:thisRandNum},function(data)
...
function delete_image1(a,b)
	{$.post(interactiveURL,{item_id:a,item_type:b,interactive:"delete_image1"},function(data) 
		{$("#delete_image_"+a).html(data).show();});}
function delete_image2(a,b)
	{$.post(url,{id:id,ids:ids,album_id:album_id,item_id:a,item_type:b,interactive:"delete_image2",thisWipit:thisRandNum},function(data) 
...
<div class="entire_bottom_media_album"><div class="image_box_wrap" id="image_box_wrap"><?php include "images_list.php"; ?></div></div>
...
<div class="bottom_wall" id="bottom_media_album">
	<?php include_once "albums_posts_content.php";?>
</div>
