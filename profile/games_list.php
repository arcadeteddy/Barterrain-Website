...
if (in_array($ids,$friend_array))
	...
	{$view_type="AND game_type!='b'";}
else {$view_type="";}

$mysql = mysql_query("SELECT id FROM games WHERE user_id=$id $view_type AND delete_item='1' ORDER BY upload_date DESC");
...
if ($numRows>0)
	...
	foreach ($gamesArray2 as $key => $value)
		{$i++;	
		$mysql_game=mysql_query("SELECT * FROM games WHERE id='$value' LIMIT 1") or die ("Sorry, we have a system error!");
		...
		$game_default_pic="../user_files/user0/default_game_cover.png";
		if(file_exists($game_pic))
			{$game_cover="<a href='#' onclick='return false' onmousedown='javascript:game_opener(".$ids.",".$id.",".$game_id.");'><img src='$game_pic' height='125px' width='213px' class='thumb_background'/></a>";}
		...
		$gamesDisplayList .= '<div class="upload_box" id="upload_game_box'.$game_id.'">
								...
								<div class="upload_box3">
									<div class="like_love" id="like_love_games'.$game_id.'">'.$like_love_game.'
									<span class="dot_divider"> &middot; </span><span class="post_date">'.$upload_date.'</span>'.$like_love2_game.'</div>
								</div>
							</div>';
		}
