...
$AlbumArray ="0";
$mysql_albums = mysql_query("SELECT id FROM albums WHERE user_id='$id' AND union_type='albums'");
...
$VideoArray ="0";
$mysql_videos = mysql_query("SELECT id FROM videos WHERE user_id='$id' AND union_type='videos'");
...
$GameArray ="0";
$mysql_games = mysql_query("SELECT id FROM games WHERE user_id='$id' AND union_type='games'");
...
////////// WALL!!! //////////
$mysql_union = mysql_query("(SELECT id, note_date AS datetime, union_type FROM notes WHERE user_id='$id' AND delete_item='1')
					UNION ALL(SELECT id, post_date AS datetime, union_type FROM posts WHERE (user_page_id='$id' OR user_post_id='$id') AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM albums WHERE user_id='$id' AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM images_walls WHERE user_id='$id' AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM videos WHERE user_id='$id' AND delete_item='1')
					UNION ALL(SELECT id, upload_date AS datetime, union_type FROM games WHERE user_id='$id' AND delete_item='1')
					...
					UNION ALL(SELECT id, create_date AS datetime, union_type FROM planets WHERE user_id='$id' AND delete_item='1') 
...
while ($row = mysql_fetch_array ($mysql1))
	...
	if($user_post_id!==$user_page_id)
		{$SeeFriendship="<span class='dot_divider'> &middot; </span><a href='#' class='post_link' onclick='return false' onmousedown='javascript:SeeFriendship(".$user_page_id.", ".$user_post_id.",".$id.",1);'>See Friendship</a>";}
	else{$SeeFriendship="";}
...
while ($row = mysql_fetch_array ($mysql1))
	...
	if (file_exists($game_pic))
		{$game_cover="<a href='#' onclick='return false' onmousedown='javascript:game_opener(".$ids.",".$id.",".$game_id.");'><img class='cover thumb_background' src='$game_pic' height='76px' width='130px'/></a>";}
...
$(window).data('ajaxready1', true).scroll(function()
	...
	if($(window).scrollTop()>=$("#bottom_wall").height()-$(document).height())
		...
		$.ajax({cache: false,url:"wall_content.php?number="+number+"&cacheBuster="+cacheBuster+"&id="+id+"&ids="+ids,
