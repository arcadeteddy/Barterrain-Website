...
$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM planets_posts WHERE (delete_item='1')
							...
							UNION ALL(SELECT id, note_date AS datetime, union_type FROM planets_notes WHERE (delete_item='1')
								AND (the_note_subject LIKE '%$search%' OR the_note LIKE '%$search%'))
							UNION ALL(SELECT id, upload_date AS datetime, union_type FROM planets_albums WHERE (delete_item='1')
							...
							UNION ALL(SELECT id, upload_date AS datetime, union_type FROM planets_games WHERE (delete_item='1')
								AND (game_name LIKE '%$search%' OR game_description LIKE '%$search%'))
							UNION ALL(SELECT id, upload_date AS datetime, union_type FROM planets_videos WHERE (delete_item='1')
							...
							UNION ALL(SELECT id, post_date AS datetime, union_type FROM planets_videos_posts WHERE (delete_item='1')
								AND (the_post LIKE '%$search%'))
							UNION ALL(SELECT id, link_create_date AS datetime, union_type FROM link_creates WHERE (delete_item='1')
...
$(window).data('ajaxready2', true).scroll(function()
	...
	if($(window).scrollTop()>=$("#bottom_search_planets").height()-$(document).height())
		{$("div#expand_bottom_box").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready2', false);
		$.ajax({cache: false,url:"search_planets_content.php?number="+number+"&cacheBuster="+cacheBuster+"&search="+search_box+"&id="+id+"&ids="+ids,
			success:function(html)
				{if(html){$("#bottom_search_planets").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_box").show();}
