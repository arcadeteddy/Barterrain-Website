...
////////// FRIENDS!!! //////////
if ($FF_array=="") {$mysql_union = mysql_query("(SELECT id, upload_date AS datetime, union_type FROM albums WHERE ((user_id='$id') AND delete_item='1')
...
else {$mysql_union = mysql_query("(SELECT id, upload_date AS datetime, union_type FROM albums WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1')
								AND (album_name LIKE '%$search%' OR album_description LIKE '%$search%'))
...
$(window).data('ajaxready13', true).scroll(function()
	{if ($(window).data('ajaxready13') == false) return;
	if($(window).scrollTop()>=$("#bottom_search_profiles_albums").height()-$(document).height())
		{$("div#expand_bottom_box").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready13', false);
		$.ajax({cache: false,url:"search_profiles_albums_content.php?number="+number+"&cacheBuster="+cacheBuster+"&search="+search_box+"&id="+id+"&ids="+ids,
			success:function(html)
				{if(html){$("#bottom_search_profiles_albums").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_box").show();}
				 $(window).data('ajaxready13', true);}
			});
		}
	});
