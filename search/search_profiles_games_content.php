...
////////// SEARCH!!! //////////
if ($FF_array=="") {$mysql_union = mysql_query("(SELECT id, upload_date AS datetime, union_type FROM games WHERE ((user_id='$id') AND delete_item='1')
								AND (game_name LIKE '%$search%' OR game_description LIKE '%$search%'))
							ORDER BY datetime DESC LIMIT ".$number.",20");}
else {$mysql_union = mysql_query("(SELECT id, upload_date AS datetime, union_type FROM games WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1')
								AND (game_name LIKE '%$search%' OR game_description LIKE '%$search%'))
							ORDER BY datetime DESC LIMIT ".$number.",20");}
...
$(window).data('ajaxready14', true).scroll(function()
	{if ($(window).data('ajaxready14') == false) return;
