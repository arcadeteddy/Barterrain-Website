...
if ($FF_array=="") {$mysql_union = mysql_query("(SELECT id, post_date AS datetime, union_type FROM posts WHERE ((user_post_id='$id') AND delete_item='1') 
							...
							UNION ALL(SELECT id, note_date AS datetime, union_type FROM notes WHERE ((user_id='$id') AND delete_item='1') 
							...
							UNION ALL(SELECT id, last_login_date AS datetime, union_type FROM members WHERE (delete_member='1')
							...
							ORDER BY datetime DESC LIMIT ".$number.",20");}
								...
								AND (the_post LIKE '%$search%'))
							UNION ALL(SELECT id, note_date AS datetime, union_type FROM notes WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1') 
								AND (the_note_subject LIKE '%$search%' OR the_note LIKE '%$search%'))
							UNION ALL(SELECT id, upload_date AS datetime, union_type FROM albums WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1')
								AND (album_name LIKE '%$search%' OR album_description LIKE '%$search%'))
							UNION ALL(SELECT id, upload_date AS datetime, union_type FROM images_walls WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1')
								AND (album_name LIKE '%$search%'))
							UNION ALL(SELECT id, upload_date AS datetime, union_type FROM videos WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1')
								AND (video_name LIKE '%$search%' OR video_description LIKE '%$search%'))
							UNION ALL(SELECT id, upload_date AS datetime, union_type FROM games WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1')
								AND (game_name LIKE '%$search%' OR game_description LIKE '%$search%'))
							...
							UNION ALL(SELECT id, post_date AS datetime, union_type FROM videos_posts WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1')
								AND (the_post LIKE '%$search%'))
							UNION ALL(SELECT id, create_date AS datetime, union_type FROM planets WHERE ((user_id IN ($FRIENDS) OR user_id='$id') AND delete_item='1')
								AND (planet_name LIKE '%$search%' OR planet_description LIKE '%$search%'))
							UNION ALL(SELECT id, last_login_date AS datetime, union_type FROM members WHERE (delete_member='1')
								AND (firstname LIKE '%$search%' OR lastname LIKE '%$search%'))
	...
	if($images_count==1)
		{foreach ($imagesArray as $key => $value)
			...
			$row=mysql_fetch_assoc($mysql_gif);
				{$ext=$row['ext'];}
			if ($ext=="gif") {$black_gif="<img class='black_gif' src='barterrain_search_images/black.png' height='415px' width='415px'/><table class='black_gif' height='415px' width='415px'><tr><td><span class='black_gif'>GIF</span></td></tr></table>";}
		...
		{foreach ($imagesArray as $key => $value)
			...
			$row=mysql_fetch_assoc($mysql_gif);
				{$ext=$row['ext'];}
			if ($ext=="gif") {$black_gif="<img class='black_gif' src='barterrain_search_images/black.png' height='203px' width='203px'/><table class='black_gif' height='203px' width='203px'><tr><td><span class='black_gif'>GIF</span></td></tr></table>";}
			$i++;$images_pic="../user_files/user$user_id/album_thumbs_$album_id/album_".$album_id."_thumb_".$value.".jpg";
		...
		{foreach ($imagesArray as $key => $value)
			...
			$row=mysql_fetch_assoc($mysql_gif);
				{$ext=$row['ext'];}
			if ($ext=="gif") {$black_gif="<img class='black_gif' src='barterrain_search_images/black.png' height='133px' width='133px'/><table class='black_gif' height='133px' width='133px'><tr><td><span class='black_gif'>GIF</span></td></tr></table>";}
			$i++;$images_pic="../user_files/user$user_id/album_thumbs_$album_id/album_".$album_id."_thumb_".$value.".jpg";
...
$(window).data('ajaxready1', true).scroll(function()
	...
	if($(window).scrollTop()>=$("#bottom_search_profiles").height()-$(document).height())
		{$("div#expand_bottom_box").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready1', false);
		$.ajax({cache: false,url:"search_profiles_content.php?number="+number+"&cacheBuster="+cacheBuster+"&search="+search_box+"&id="+id+"&ids="+ids,
			success:function(html)
				{if(html){$("#bottom_search_profiles").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_box").show();}
