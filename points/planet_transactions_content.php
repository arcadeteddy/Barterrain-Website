...
$mysql_union = mysql_query("(SELECT * FROM point_transactions WHERE (plus_id='$ids' OR minus_id='$ids') AND delete_transaction='1' AND create_type='planet') 
...
if ($union_type=="transactions")
	...
	else if ($transaction_type=="link_creates") {$transaction_type="Planet Links";$tt_text="This";}
	else if ($transaction_type=="member_posts")
	...
	if ($minus_id==$ids) 
		...
		if ($creator_array!="") 
			...
			else if ($alias_activation=="0") {$plus_text="<a href='../profile/profile.php?id=".$plus_id."' class='body'>".$firstname." ".$lastname."</a>";}
			else {$plus_text="<a class='alias_body_link' href='#' onclick='return false'>".$alias."</a>";}}
		else if ($creator_display=="0")
			{if ($alias_activation=="0") {$plus_text="<a href='../profile/profile.php?id=".$plus_id."' class='body'>".$firstname." ".$lastname."</a>";}
			else {$plus_text="<a class='alias_body_link' href='#' onclick='return false'>".$alias."</a>";}}
		...
		if ($transaction_type=="images")
			...
			$link_text="<a href='#' onclick='return false' class='body display_image_button' onmousedown='javascript:display_image(".$user_page_id.",".$ids.",".$item_id.");'>".$link_text."</a>";}
		else 
		...
		if ($creator_array!="") 
			...
			else if ($alias_activation=="0") {$minus_text="<a href='../profile/profile.php?id=".$minus_id."' class='body'>".$firstname." ".$lastname."</a>";}
			else {$minus_text="<a class='alias_body_link' href='#' onclick='return false'>".$alias."</a>";}}
		else if ($creator_display=="0")
			{if ($alias_activation=="0") {$minus_text="<a href='../profile/profile.php?id=".$minus_id."' class='body'>".$firstname." ".$lastname."</a>";}
			else {$minus_text="<a class='alias_body_link' href='#' onclick='return false'>".$alias."</a>";}}
		...
		if ($transaction_type=="images")
			...
			{$link_text="<a href='#' onclick='return false' onmousedown='javascript:display_item(".$ids.",".$item_id.",".$item_type.",".$create_id.",".$create_type_json.");' class='body display_button'>".$link_text."</a>";}
		}
...
$(window).data('ajaxready2', true).scroll(function(e)
	{if ($(window).data('ajaxready2') == false) return;
	if($(window).scrollTop()>=$("#bottom_planet_transactions").height()-$(document).height())
		{$("div#expand_bottom_point_transactions").hide();
		$("div#load_content_scroll").show();
		$(window).data('ajaxready2', false);
		$.ajax({cache: false,url:"planet_transactions_content.php?number="+number+"&id="+id+"&ids="+ids+"&list_number="+list_number,
			success:function(html)
				{if(html){$("#bottom_planet_transactions").append(html);$("div#load_content_scroll").hide();$("div#expand_bottom_point_transactions").show();}
